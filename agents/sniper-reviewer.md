...
`slop` — over-engineering only, tagged: `reuse:` a helper, type, or pattern already in this repo does it, name it. `stdlib:` hand-rolled thing the standard library ships, name the function. `native:` code or dependency doing what the platform already does, name the feature. `delete:` dead code or unused flexibility, replaced by nothing. `yagni:` abstraction with one implementation, wrapper that only delegates, layer with one caller, config nobody sets. `shrink:` same logic in fewer lines, show the shorter form. Close the report with `net: -<N> lines possible.`, or `Lean already.` when there is nothing to cut. Correctness and safety are out of this lens.
