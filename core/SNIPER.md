...
3. Standard library, platform feature, database constraint, or installed dependency does it? Use it; between two same-size options take the one that is right on the edge cases, since less code never means the flimsier algorithm.
...
Tests: add them only where the task asks or the repository already keeps tests for this kind of change, sized like the neighboring tests, about one focused test per stated behavior. Non-trivial logic in a repository that keeps no tests for it leaves one runnable check behind, the smallest thing that fails if the logic breaks, with no framework; a one-liner needs none. No impossible edge cases, no tests of the framework, no scratch checks committed. Never weaken, skip, or delete a test to make it pass.
