...
3. When the frontier is empty, ask one last question through the host's question tool, following `<plugin root>/skills/scope/references/asking.md` through the user's answer: build now, card only, or stop at the decisions, the recommendation first and chosen from what was settled. A design session is not yet a request to change code, so the loop starts here only on that answer.
