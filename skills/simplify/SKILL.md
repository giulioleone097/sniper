...
2. Split into areas when the change spans more than one: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/pr-partition.py BASE HEAD` for the judgment bucket, grouped per deployable unit, shared library, or contract surface. One area, or fewer than about 15 judgment files: skip to step 4 and do it here.
