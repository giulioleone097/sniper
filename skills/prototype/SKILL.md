...
7. Report the verdict in this shape, then feed the proven answer into `atlasme` or `scope` (Skill tool `atlas:atlasme` / `atlas:scope`, `$atlasme` / `$scope` on Codex) so the settled design continues as a normal request:
...
next: atlasme <question> | scope <question> | none
```
