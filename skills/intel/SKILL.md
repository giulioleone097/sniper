...
2. Spin up a background subagent to do the reading, so the session keeps working while it investigates. This is not `atlas-scout`, which is code-read-only for this repo; give the question to the host's general-purpose or web-capable subagent instead. Hand it the question and steps 3-4 as its job.
