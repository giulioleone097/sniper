...
4. Print the settled tree exactly as the reference shows it, reach and risk per decision and the map when it applies. Card only, Stop, or `--out` given: write the tree to `--out <file>`, else `docs/grill-<yyyy-mm-dd>-<slug>.md`, headed `# Grill: <subject>` with a `next:` line, and print the path; `scope <file>` resumes from it. Then, on build now, invoke `scope` on the settled request (Skill tool `sniper:scope`, `$scope` on Codex); on card only, invoke `scope --card-only`.
