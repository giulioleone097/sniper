...
| `scope` | `intake.md` (issues, PRs, reports, images, handoff and atlasme files), `atlasme.md`, `asking.md` | card emitted and handed to build; no argument and no request in the conversation offers the latest handoff |
...
| `atlasme` | scope's `intake.md` for a document subject, `atlasme.md`, `asking.md`, ship's `shapes.md` for the map | settled tree with reach and risk per decision, and the map when more than one domain is reached, printed and, on card only, stop or `--out`, written to disk; chosen next step run |
...
Every stage follows `skills/scope/references/asking.md` for tool selection, payload shape and the full interaction lifecycle. Use a permitted blocking tool, or `request_user_input_async` with an interruptible active wait. An asynchronous acknowledgement only opens the question: the turn stays active until the user's reply or explicit cancellation, including when a stage says "ask and stop". Numbered text is the fallback when the host cannot sustain that interaction. The core carries this invariant so callers do not each implement their own waiting rule.
