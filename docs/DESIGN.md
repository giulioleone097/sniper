...
sniper/
  ...
  skills/<stage>/SKILL.md           5 stages and 9 named entry points (atlas, simplify, handoff, optimize, intel, question, howto, prototype, improve), each under 120 lines
...
| `scope` | `intake.md` (issues, PRs, reports, images, handoff and atlas files), `grill.md`, `asking.md` | card emitted and handed to build; no argument and no request in the conversation offers the latest handoff |
...
| `atlas` | scope's `intake.md` for a document subject, `grill.md`, `asking.md`, ship's `shapes.md` for the map | settled tree with reach and risk per decision, and the map when more than one domain is reached, printed and, on card only, stop or `--out`, written to disk; chosen next step run |
