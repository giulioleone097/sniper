...
Un plugin per Claude Code e Codex che porta un lavoro dall'arrivo (issue, PR, idea) alla consegna (commit, PR, dossier) attraverso sedici skill piccole, quattro agenti e cinque rilevatori, con una dottrina anti-slop iniettata a ogni sessione.
...
- Rilevatori: `scripts/checks.sh` (comandi di verifica del progetto), `tracker.sh` (forge e CLI), `consumers.sh` (repository dipendenti), `tokens.sh` (token di design), `repo-facts.sh` (fatti per la mappa), `pr-partition.py` (partizione del diff); `skills/narrate/scripts/` tiene `pr-contracts.py`, `pr-walkthrough.py`, `test-summary.py`.
