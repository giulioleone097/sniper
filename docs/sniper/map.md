...
- Fasi: `skills/<stage>/SKILL.md` (<= 120 righe, router; descrizione che apre con "Use when", <= 70 parole) con `agents/openai.yaml` per Codex e `references/` (<= 80 righe) per ogni ramo: intake, grill e asking in scope; plan, debug, prove e i modi in build; audit e platform-native in review; narrate, shapes, posting, learn, environment e handoff in ship; map in setup. Raggiunte dai due host per nome e l'una dall'altra; Codex accorcia la descrizione a ~45 caratteri.
...
## Flusso principale
```
setup? -> scope (intake | grill | card) -> build (plan? | debug? | modo | prove) -> review (shrink, revisori per area, integratore) -> ship (commit, dossier, lesson, handoff)
```
Il ciclo si chiama da solo: ogni fase invoca la successiva; `review` dispaccia un revisore per area e un integratore che verifica e attribuisce le regressioni alla baseline.
