...
| mattpocock/skills | https://github.com/mattpocock/skills | 2026-09 | `grilling`: a design tree worked in rounds, whole frontier asked at once, every question carrying a recommendation, facts found by the agent → `grill`. `handoff`: compact a session for the next agent, point at artifacts instead of restating them → `handoff`. `retro`: improve the environment (navigation, automated checks, review rules, steering weight, tool economy, information access) → `learn/references/environment.md`. `tdd`: red before green, the failing run as the last chance to catch a wrong test → one clause in `build`. Rejected: `triage`, `to-spec`, `to-tickets`, `wayfinder` (tracker-coupled; `dps-ado` and `narrate` own that side), `research` (a scout plus the host's fetch tools already do it), `prototype` (throwaway spikes are product design, outside this cycle), `teach`, `wait-what`, `writing-*` (not development stages) |
