...
- Installer user-level: `scripts/install-devin.sh` e `scripts/install-cursor.sh` (skill come `atlas-<stage>` con riferimenti riscritti, agenti, dottrina/hook, idempotenti e reversibili con `--remove`) per dove il plugin manager non arriva.
