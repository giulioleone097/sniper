...
Each cell is one headless Claude Code session (`claude -p`) in a temporary workspace, run with `--bare` (no user settings, memory, hooks or other plugins). Arms use no plugin (`baseline`), the current repository (`sniper`), and optionally `--previous-plugin` (`previous`). Plugin arms load that directory's skills and doctrine. The same runner, model argument, tasks and limits apply to every arm. Files are scored deterministically, stdlib only.
...
python3 evals/run.py --previous-plugin /path/to/previous-sniper --model MODEL --runs 3
