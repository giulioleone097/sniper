...
root=$(cd "${1:-.}" 2>/dev/null && pwd -P) || { echo "none=1"; exit 0; }
...
[ -n "$files" ] && {
  section custom-properties "$(cat $files 2>/dev/null | grep -ohE '(^|[;{[:space:]])--[a-zA-Z0-9_-]+:[[:space:]]*[^;{}]{1,60};' | grep -vE '^[;{[:space:]]*--tw-' | sed -e 's/^[;{[:space:]]*//' -e 's/;$//' -e 's/:[[:space:]]*/: /' | top 30)"
