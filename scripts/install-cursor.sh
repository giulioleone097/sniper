#!/bin/sh
# sniper install-cursor: user-level install for Cursor, for machines where the
...
if [ "$1" = "--remove" ]; then
  for d in "$CURSOR_DIR"/skills/sniper-*/; do
    [ -d "$d" ] && rm -rf "$d"
  done
  rm -f "$CURSOR_DIR"/agents/sniper-*.md
...
PYEOF
  echo "sniper: removed from $CURSOR_DIR"
  exit 0
fi
...
for d in "$ROOT"/skills/*/; do
  stage="$(basename "$d")"
  dest="$CURSOR_DIR/skills/sniper-$stage"
  rm -rf "$dest"
  cp -R "$d" "$dest"
